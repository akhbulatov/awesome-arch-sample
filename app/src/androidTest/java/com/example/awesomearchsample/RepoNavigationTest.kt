...
import com.example.awesomearchsample.feature.user.userdetails.USER_DETAILS_SCREEN_TAG
...
class RepoNavigationTest {
    ...
    @Test
    fun appStart_repoDetailsAuthorNavigatesToUserDetails() {
        composeRule.waitUntil(timeoutMillis = 10_000) {
            composeRule.onAllNodesWithTag(REPOS_SCREEN_TAG).fetchSemanticsNodes().isNotEmpty()
        }
        ...
        composeRule.waitUntil(timeoutMillis = 10_000) {
            composeRule.onAllNodesWithTag(REPO_DETAILS_AUTHOR_TAG).fetchSemanticsNodes().isNotEmpty()
        }

        composeRule.onNodeWithTag(REPO_DETAILS_AUTHOR_TAG).performClick()

        composeRule.waitUntil(timeoutMillis = 10_000) {
            composeRule.onAllNodesWithTag(USER_DETAILS_SCREEN_TAG).fetchSemanticsNodes().isNotEmpty()
        }

        composeRule.onNodeWithTag(USER_DETAILS_SCREEN_TAG).assertExists()
    }
}
